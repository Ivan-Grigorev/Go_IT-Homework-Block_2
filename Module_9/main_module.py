...
from address_book import Session
    ...
    while True:
        try:
            ...
            if sep_command[0] == 'add' and sep_command[1] == 'contact':
                ...
                if len(address) == 0:
                    address += '-'
                address_book = AddressBook(name, phone, email, address)
                session.add(address_book)

            elif sep_command[0] == 'show' and sep_command[1] == 'contact':
                address_book_data = session.query(AddressBook).filter_by(name=sep_command[2].title())
                for row in address_book_data:
                    print(row)

            elif sep_command[0] == 'show' and sep_command[1] == 'all':
                for address_book in session.query(AddressBook).all():
                    print(address_book)
                ...
                session.query(AddressBook).filter(AddressBook.name == edited_name).update({edited_data: new_data})

            elif sep_command[0] == 'delete' and sep_command[1] == 'contact':
                deleted_name = input("Enter name to delete, please\n>>").title()
                answer = input("Are you sure? Y/N\n>>")
                if answer.title() == 'Y':
                    session.query(AddressBook).filter(AddressBook.name == deleted_name).delete()
                else:
                    continue
